...
function is31Flavors(input_array){
    return input_array.length == 31;
}
...
function getFlavorByIndex(input_array, i){
...
function removeFlavorByName(flavor_array, bad_flavor){
    let bad_flavor_idx = flavor_array.indexOf(bad_flavor);
    flavor_array.splice(bad_flavor_idx, 1);
    return flavor_array;
}
...
function getAverageWordLength(input_array){
    let word_reducer = function(acc, str) {
        return acc + str.split(' ').length;
    }
    return input_array.reduce(word_reducer, 0) / input_array.length;
}
